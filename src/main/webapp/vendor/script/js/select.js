    ...
    mySelect.prototype = {
        ...
        initOption: function () {
            ...
            for (var i = 0; i < this.options.option.length; i++) {
                this.ele.find(".mySelect-option").append('<div data-value="' + this.options.option[i].value + '" id="selected' + this.options.option[i].value + '"><span>' + this.options.option[i].label + '</span><i class="fa fa-check"></i></div>')
            }
        },
        addEvent: function () {
            var that = this;
